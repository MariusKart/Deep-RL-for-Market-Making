...
def rollout(policy, starting_inventory, horizon, market, r, Sigma, lb , ub , avg_sizes):
    """
    Stepwise rollout faithful to the paper's logic.

    Returns (flattened):
      S        : (N, d)   actual visited states q_t
      A_base   : (N, 2)   [bond_id, p_base]
      A_noisy  : (N, 2)   [bond_id, p_noisy]
      D        : (N,)     direction (1=bid, 0=ask)
      R        : (N,)     expected immediate reward
      S_trade  : (N, d)   executed-branch state q_exec
    """
    ...
    Anoise_out = np.empty((T, B, 2), dtype=np.float32)
    ...
    rng = getattr(market, "rng", np.random.default_rng())
    ...
    with torch.inference_mode():
        for t in range(T):
            ...
            # Draw one RFQ per batch element, step by step
            ...
            # If you want the paper's constant-per-bond RFQ sizes, use this instead:
            # sz_t = avg_sizes_vec[i_t]
            ...
            # Noisy action in probability space
            eps = rng.uniform(-0.05, 0.05, size=B).astype(np.float32)
            a_noisy = np.clip(a_base + eps, NU, 1.0 - NU)

            # Convert p_base -> delta_base
            delta_base = np.empty(B, dtype=np.float32)
            for j in uniq:
                m = (i_t == j)
                delta_base[m] = np.asarray(market.inv_f(int(j), a_base[m]), dtype=np.float32)

            # Executed branch q_exec from current state only
            trade_sign = np.where(dir_t == 1, +1.0, -1.0).astype(np.float32)
            q_exec = q.copy()
            q_exec[np.arange(B), i_t] += trade_sign * sz_t

            # Store executed branch for TD / actor targets
            # Keeping the raw executed branch is cleaner than clipping
            # but if your downstream code expects admissible inventories, clip here:
            ...
            # - set p_eff=0 when infeasible
            # - sample execution with p_eff
            # - return realized next inventory
            ...
            Anoise_out[t, :, 0] = i_t.astype(np.float32)
            Anoise_out[t, :, 1] = a_noisy
            D_out[t] = dir_t
    ...
    Anoise_flat = Anoise_out.reshape(N, 2)
    ...
    return S_flat, Abase_flat, Anoise_flat, D_flat, R_flat, Strade_flat
